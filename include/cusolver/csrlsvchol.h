...
inline void
csrlsvchol(sparse::handle_t h, int m, int nnz, const matrix_description descr_a,
		const float* values, const int* starts, const int* indices,
		const float* b, float tol, int reorder, float* x, int* singularity)
{
	throw_if_error(cusolverSpScsrlsvchol(h, m, nnz, descr_a, values, starts,
...
{
	throw_if_error(cusolverSpDcsrlsvchol(h, m, nnz, descr_a, values, starts,
		indices, b, tol, reorder, x, singularity));
}
