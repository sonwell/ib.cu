...
namespace impl {
	...
	get_colors(const views_type& views, int colors = 2)
	{
		...
		{
			auto gridpts = view.gridpts();
			return view.solid_boundary ||
				gridpts <= 1 ||
				(gridpts - 1) % colors != 0;
		};
...
private:
	mem::device_ptr<int> permutation;
	mem::device_ptr<int> inverse;
protected:
	using coloring::permute_with;
public:
	virtual lwps::matrix
	permute(const lwps::matrix& m) const
	{
	...
	virtual lwps::matrix
	unpermute(const lwps::matrix& m) const
	{
	...
	virtual lwps::vector
	permute(const lwps::vector& v) const
	{
		auto* pdata = inverse.data();
	...
	virtual lwps::vector
	unpermute(const lwps::vector& v) const
	{
		auto* pdata = permutation.data();
		return permute_with(v, pdata);
	}
		...
		permutation(coloring::size()), inverse(coloring::size())
	{
		...
		mem::device_ptr<int> starts(colors+1);
