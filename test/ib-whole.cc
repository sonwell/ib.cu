...
decltype(auto)
boundary_conditions(const grid_type& grid, const domain_type& domain,
		units::unit<0, 0, -1> shear_rate)
{
	...
	std::get<2>(ub) = b * vector{fd::size(g, y), algo::fill(twv)};
	return ub;
}
...
{
	using fd::correction::second_order;
	auto ub = boundary_conditions(grid, domain, shear_rate);
...
main(int argc, char** argv)
{
	...
	endothelium endothelium{4096, 10000, sharp};
	...
	auto forces = [&, &st=st] (units::time t, const auto& v)
	{
		using namespace util::functional;
		units::time dt = t - st.t;
