			...
			const reference_type& ref, units::unit<0, 0, -1> shear_rate)
{
	...
	auto n = ref.num_data_sites;
	auto p = traits::sample(n);
	auto x = traits::shape(p);
...
main(int argc, char** argv)
{
	...
	constexpr forces::tether tether{1e-2_dyn/1_cm};
	constexpr forces::hookean tension{2.5e-2_dyn/1_cm};
	constexpr forces::combine forces{tether, tension};
	...
	util::logging::info("tether info: modulus = ", tether.modulus);
	...
	endothelium ref{4096, 4900, basic, smooth};
